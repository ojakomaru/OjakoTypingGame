    ...
    <>
      {fields.map((item, index) => (
        <Stack mt={2} key={item.id}>
          ...
          <IconButton color="warning" aria-label="delete" onClick={() => remove(index)}>
            <DeleteOutlineIcon />
          </IconButton>
        </Stack>
      ))}
      <Stack direction="row" justifyContent="space-evenly" alignItems="baseline">
        <Button sx={{ mt: 1 }} startIcon={<AddIcon />} onClick={() => append({ text: '' })}>
          {'問題文を追加'}
        </Button>
        <IconButton color="warning" aria-label="delete" onClick={() => remove()} sx={{ fontSize: '0.8rem' }}>
          <DeleteOutlineIcon />
          {'すべての問題を削除'}
        </IconButton>
      </Stack>
